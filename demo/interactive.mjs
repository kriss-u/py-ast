...
class InteractiveExplorer {
  ...
  showJSON(args) {
    ...
    try {
      ...
      if (depth !== undefined) {
        ...
        jsonOutput = pretty ? JSON.stringify(limitedAST, null, 2) : JSON.stringify(limitedAST);
      } else {
        jsonOutput = pretty ? JSON.stringify(this.currentAST, null, 2) : JSON.stringify(this.currentAST);
      }
  ...
  async saveJSON(args) {
    ...
    try {
      const jsonOutput = pretty ? JSON.stringify(this.currentAST, null, 2) : JSON.stringify(this.currentAST);
  ...
  showExamples() {
    ...
    const examples = [
      ...
      { name: 'Function definition', code: 'def greet(name):\n    return f"Hello, {name}!"' },
      { name: 'Class definition', code: 'class Person:\n    def __init__(self, name):\n        self.name = name' },
      { name: 'List comprehension', code: '[x**2 for x in range(10) if x % 2 == 0]' },
      { name: 'Dictionary', code: '{"name": "Alice", "age": 30, "city": "NY"}' },
