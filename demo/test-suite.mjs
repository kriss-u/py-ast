...
`;
	...
	try {
		...
		visitor.functionCalls.forEach((call) => {
			...
			console.log(
				`  - ${objStr}${call.name}(${call.args} args) at line ${call.lineno}`,
			);
		});
...
function testPerformance() {
	...
	sizes.forEach((size) => {
		...
		try {
			...
			console.log(
				`📏 ${size} lines: ${time}ms, ${nodeCount} nodes (${(nodeCount / time).toFixed(1)} nodes/ms)`,
			);
		} catch (error) {
...
function runTests() {
	...
	if (results.errors) {
		console.log(
			`⚠️  Error handling: ${results.errors.errorsHandled} errors properly caught`,
		);
	}
