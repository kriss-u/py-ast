  ...
  for (const node of walk(ast)) {
    if (node.nodeType === 'FunctionDef') {
      functions.push({
        ...
        docstring: node.body[0]?.nodeType === 'Expr' && 
  ...
  function simplifyNode(node, maxDepth = 3, currentDepth = 0) {
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      if (Array.isArray(value)) {
        if (value.length > 0 && value[0] && typeof value[0] === 'object' && value[0].nodeType) {
          simplified[key] = value.map(item => simplifyNode(item, maxDepth, currentDepth + 1));
      ...
      } else if (!['lineno', 'col_offset', 'end_lineno', 'end_col_offset'].includes(key)) {
        simplified[key] = value;
      }
    }
