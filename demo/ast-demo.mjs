...
class ASTStatsVisitor extends NodeVisitor {
	...
	visit(node) {
		...
		this.stats.nodeTypes.set(
			nodeType,
			(this.stats.nodeTypes.get(nodeType) || 0) + 1,
		);
		...
		if (nodeType === "FunctionDef" || nodeType === "AsyncFunctionDef") {
			this.stats.functionDefs.push({
				...
				hasAnnotations:
					!!node.returns || node.args.args.some((arg) => arg.annotation),
			});
		}
		...
		if (nodeType === "ClassDef") {
			this.stats.classDefs.push({
				...
				methods: node.body.filter(
					(stmt) =>
						stmt.nodeType === "FunctionDef" ||
						stmt.nodeType === "AsyncFunctionDef",
				).length,
				hasDecorators: node.decorator_list.length > 0,
			});
		}
	...
	getReport() {
		const nodeTypesArray = Array.from(this.stats.nodeTypes.entries()).sort(
			(a, b) => b[1] - a[1],
		);
...
async function analyzeFile(filePath) {
	try {
		...
		if (docstring) {
			console.log(
				`📝 Module docstring: "${docstring.slice(0, 100)}${docstring.length > 100 ? "..." : ""}"`,
			);
		}
		...
		if (stats.functions.length > 0) {
			...
			stats.functions.forEach((func) => {
				...
				console.log(
					`  Line ${func.lineno}: ${func.name}(${func.args} args)${featuresStr}`,
				);
			});
		}
			...
			stats.imports.forEach((imp) => {
				if (imp.type === "ImportFrom") {
					const names = imp.names
						...
						.join(", ");
					console.log(
						`  Line ${imp.lineno}: from ${imp.module || "?"} import ${names}`,
					);
				} else {
					const names = imp.names
						.map((n) => (n.asname ? `${n.name} as ${n.asname}` : n.name))
	...
	try {
		...
		if (results.length > 0) {
			...
			const totalNodes = results.reduce(
				(sum, r) => sum + r.stats.summary.totalNodes,
				0,
			);
			const totalFunctions = results.reduce(
				(sum, r) => sum + r.stats.summary.functions,
				0,
			);
			const totalClasses = results.reduce(
				(sum, r) => sum + r.stats.summary.classes,
				0,
			);
			...
			console.log(
				`⚡ Average parse time: ${(totalParseTime / results.length).toFixed(2)}ms`,
			);
			console.log(
				`📈 Average nodes per file: ${Math.round(totalNodes / results.length)}`,
			);
		}
